...
parse_args() {
          ...
          -)
              case "${OPTARG}" in
                  ...
                  version-prefix)
                      val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                      #echo "Parsing option: '--${OPTARG}', value: '${val}'" >&2
                      putit_artifact_semver_prefix=${val}
                      ;;  
                  version-build)
                      val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
                      #echo "Parsing option: '--${OPTARG}', value: '${val}'" >&2
                      putit_artifact_semver_build=${val}
                      ;;  
                  properties)
                      val="${!OPTIND}"; OPTIND=$(( $OPTIND + 1 ))
  ...
  for key in "${!putit_artifact_properties[@]}"; do 
    ...
    #log "DEBUG" "Added property ${key}=${value} to properties file: $putit_artifact_properties_file."
    #echo "$key=$value" >> $putit_artifact_properties_file
    echo $key
    echo $value
  done | 
