...
parse_args() {
  ...
  # env need to be added to REST 
  if [ -z ${putit_release_name+x} ] || [ -z ${putit_application_name+x} ] || [ -z ${putit_application_version+x} ] || [ -z ${putit_environment_name} ] || [ -z ${putit_change_name} ]; then 
    get_help
    exit 1
  fi 
}
